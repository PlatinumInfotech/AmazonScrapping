...
def scrape_single():
    ...
    try:
        with sync_playwright() as p:
            ...
            try:
                availability_text = page.locator("#availability").first.inner_text().lower()

                if "unavailable" in availability_text or "currently unavailable" in availability_text:
            ...
            # Only extract price if product is available
            if availability == "Available":
                ...
                if price == "Not found":
                    try:
                        ...
                        fraction = page.locator("span.a-price-fraction").first.inner_text().strip()
            ...
            # --- CONSOLIDATED OFFER / BADGE / COUPON EXTRACTION ---
            coupon_text = "Not found"
            try:
                coupon_el = page.locator(".couponLabelText, #couponTextFeature_feature_div").first
                if coupon_el.count() > 0 and coupon_el.is_visible():
                    raw_coupon = coupon_el.inner_text().strip()
                    # Clean trailing noise like "Terms", "|", or "Shop items"
                    coupon_cleaned = re.sub(r'\s*(Terms|Shop items|\|).*$', '', raw_coupon, flags=re.IGNORECASE).strip()
                    if coupon_cleaned:
                        coupon_text = coupon_cleaned
                else:
                    # Fallback for generic coupon elements
                    fallback = page.locator("i.newCouponBadge, .promoPriceBlockMessage, span[id*='couponText']").first
                    if fallback.count() > 0:
                        parent_text = fallback.locator("xpath=..").inner_text()
                        match = re.search(r'(Apply\s+[\d%₹\$\s\w]+coupon|[\d%₹\$]+ off coupon)', parent_text, re.IGNORECASE)
                        if match:
            ...
            # Single Prioritized "Offer" Assignment
            offer = "Not found"
            ...
            #         "#productDetails_detailBullets_sections1, #prodDetails, #detailBulletsWrapper_feature_div"
            #     ).first
            ...
            #     if m:
            #         manufacturer = m.group(1).split("\n")[0].strip()

            #     m = re.search(r"Manufacturer Contact\s*Information\s*[:\-]?\s*(.*)", details_text)
            #     if m:
            #         manufacturer_contact = m.group(1).split("\n")[0].strip()

            #     m = re.search(r"Packer Contact\s*Information\s*[:\-]?\s*(.*)", details_text)
            #     if m:
            #         packer_contact = m.group(1).split("\n")[0].strip()

            #     m = re.search(r"Importer Contact\s*Information\s*[:\-]?\s*(.*)", details_text)
            #     if m:
            #         importer_contact = m.group(1).split("\n")[0].strip()
            ...
            return jsonify({
                ...
                "Price": price,
                "Offer": offer,  # <--- Consolidated single offer field                
